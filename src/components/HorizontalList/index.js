...
export default class HorizontalList extends PureComponent {
  ...
  componentDidMount() {
    const { categoryId, type, index } = this.props;
    if (!categoryId && type === "article") {
      GraphqlAPI.getArticles({ index })
      .then(data => this.setState({newList: data.list}))
    } else {
      GraphqlAPI.getProductsByCollection({
        categoryId,
      }).then(data => this.setState({newList: data.list}))
    }
